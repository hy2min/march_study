...
sorted(arr) # 원본데이터 유지하면서 정렬된 값을 반환
ans = sorted(arr, reverse=True)
print(ans)
arr.sort() # 원본데이터 정렬 (원본데이터 유지 안됨)
print(arr)
...
ret = sorted(arr, key=lambda x:x)
print(ret)
# sorted(정렬시킬객체, key=정렬기준이 될 함수정의)
...
ret = sorted(arr, key=lambda x:(len(x), x))
print(ret)
...
# 출력결과: ['G', 'F', 'E', 'C', 'B', 'B', 'A', 'DD', 'DB', 'DA', 'BB', 'AA']
print("4번------------")
# 4. 정답을 아래에 적어 주세요

ret = sorted(arr, key=lambda x:(-len(x), x), reverse=True)
print(ret)
...
ans = sorted(arr, key=lambda x:(-arr.count(x), x)) # 시간 복잡도가 O(n^2logn)이라 시간초과가 날 수 있다.
print(ans)
