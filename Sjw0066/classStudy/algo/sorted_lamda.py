...
answer=sorted(arr,key=lambda x:x)
print(answer)
...
print("4번------------")
# 4. 정답을 아래에 적어 주세요
answer=sorted(arr,key=lambda x:(-len(x),x),reverse=True)
print(answer)
...
print("7번------------")
# 7. 정답을 아래에 적어 주세요
answer=sorted(arr,key=lambda x:(x%2==0,x),reverse=True)
print(answer)
...
answer=sorted(arr,key=lambda x:(-arr.count(x),x))
print(answer)
# 추천 x count의 시간 복잡도가 On, sorted는 nlogn 그래서 시간복잡도가 창남
# 그래서 collections 의 counter 모듈 사용해야함
from collections import Counter
count=Counter(arr)
answer2=sorted(arr,key=lambda x:(-count[x],x))
print(answer2)
